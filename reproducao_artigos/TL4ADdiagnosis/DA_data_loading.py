...
def my_data_list(data_path, img_format='.npy'):
    ...
    Looks specifically inside 'CN' and 'AD' subfolders.
    """
    # FIX: Busca dentro das subpastas AD e CN
    CN = gg(data_path + '/CN/*' + img_format)
    AD = gg(data_path + '/AD/*' + img_format)
    
    # Fallback para minúsculas se necessário
    if len(CN) == 0:
        CN = gg(data_path + '/cn/*' + img_format)
    if len(AD) == 0:
        AD = gg(data_path + '/ad/*' + img_format)
    ...
    if len(CN) == 0 or len(AD) == 0:
        print(f'Empty folder at {data_path}. Please check if "AD" and "CN" folders exist and contain .npy files.')
        sys.exit()
    return CN, AD
...
def my_augmented_data_finder(train_list, augmented_dir, img_format='.npy'):
    ...
    for f in train_list:
        ...
        # FIX: Busca profunda (/*/*/) para encontrar arquivos dentro de affine/AD, noise/CN, etc.
...
def resize_data_volume_by_scale(data, scale):
    ...
    if isinstance(scale, float):
        ...
        scale_list = scale
    # FIX: Removido .interpolation (deprecado)
...
class MyDataLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if len(batch) == 2:
            ...
            x_batch = batch
            # Lógica original mantida: assume que arquivos começam com '2' (AD) ou '0' (CN)
            # Como mantivemos os nomes originais dos arquivos .nii, isso deve funcionar.
