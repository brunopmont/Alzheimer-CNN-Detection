...
def extract_embedding(model, data_dir, saver_dir):
    ...
    if model in models_A:
        ADnet_path = os.path.join(os.getcwd(), 'ADnet')
        if not os.path.exists(ADnet_path):
             git.Repo.clone_from('https://github.com/rturrisige/3D_CNN_pretrained_model.git', ADnet_path)
        sys.path.append(ADnet_path)
        
        original_dir = os.getcwd()
        os.chdir(ADnet_path) 
        from extract_embeddings_AD_pretrained import extract_embedding_AD
        from AD_pretrained_utilities import CNN_8CL_B, CNN
        w = torch.load('AD_pretrained_weights.pt') 
        ...
        embedding = 6
        extract_embedding_AD(data_dir, embedding, saver_dir, net, device, processing=True)
    else:
        if model == 'ResNet18':
        ...
        with alive_bar(len(files), bar='classic', spinner='arrow') as bar:
            for f in files:
                ...
                x = torch.tensor(x).float()[None, None, :, :, :]
...
if __name__ == '__main__':
    ...
    extract_embedding(args.model, args.data_dir, args.saver_dir)