...
def save_predictions(y_true, y_pred, directory='predictions', fold_num=1):
    # Garante que o diret�rio de destino exista antes de salvar
    os.makedirs(directory, exist_ok=True)
    
    np.save(os.path.join(directory, f"y_true_fold_{fold_num}.csv"), y_true)
    np.save(os.path.join(directory, f"y_pred_fold_{fold_num}.csv"), y_pred)
    print(f"Predicoes (salvas em: {os.path.abspath(directory)}")
