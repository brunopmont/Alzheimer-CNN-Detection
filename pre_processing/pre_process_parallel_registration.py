...
if __name__ == "__main__":
    ...
    DIR_RAW = f"{DIR_BASE}/ADNI_RAW"
    DIR_OUTPUT = f"{DIR_BASE}/ADNI_PROCESSED"
    ...
    for sub in os.listdir(DIR_RAW):
        for name in os.listdir(f"{DIR_RAW}/{name}"): #itera as subpastas dentre o diretório original
            input_path = f"{DIR_RAW}/{sub}/{name}"
            output_path = f"{DIR_OUTPUT}/{sub}/{name}"
            ...
            with ProcessPoolExecutor(max_workers=32) as executor: #max_workers define o número máximo de processos paralelos
